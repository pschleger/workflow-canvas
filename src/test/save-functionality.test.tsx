...
describe('Save Functionality Tests', () => {
  ...
  describe('StateEditor Save Functionality', () => {
    ...
    it('should save changes when save button is clicked', async () => {
      ...
      // Verify the StateEditor shows the state name editing interface
      expect(screen.getByText('Edit State Name')).toBeInTheDocument()
      expect(screen.getByText('Initial State')).toBeInTheDocument()

      // Click the main save button (should save with current name)
      const saveButton = screen.getByText('Save')
      await user.click(saveButton)

      // Verify onSave was called with the current definition (preserving transitions)
      expect(mockOnSave).toHaveBeenCalledWith('test-state', mockStateDefinition)
      expect(mockOnClose).toHaveBeenCalled()
    })

    it('should preserve existing transitions when saving', async () => {
      const user = userEvent.setup()
      render(
        <StateEditor
          stateId="test-state"
      ...
      // Verify the StateEditor shows the correct interface
      expect(screen.getByText('Edit State Name')).toBeInTheDocument()
      ...
      // Verify onSave was called with the definition preserving transitions
      expect(mockOnSave).toHaveBeenCalledWith('test-state', mockStateDefinition)
      expect(mockOnClose).toHaveBeenCalled()
    })
  })
  ...
  describe('Bug Fix Verification', () => {
    it('should save name changes and preserve existing state structure (StateEditor)', async () => {
      const user = userEvent.setup()
      const mockStateDefinition: StateDefinition = {
        name: 'Original State',
        transitions: []
      ...
      // Verify that the state structure is preserved
      expect(mockOnSave).toHaveBeenCalledWith('test-state', mockStateDefinition)
      expect(mockOnClose).toHaveBeenCalled()
    })
